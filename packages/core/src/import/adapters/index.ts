/*
  ...
  file. ThinkOrSwim leads because its fingerprint is document-level. The
  statement adapter runs BEFORE the deals adapter on purpose: an MT5
  history report carries both a Positions and a Deals section, and
  Positions must win (its S/L column yields calculated R); a tester report
  has no Positions section, so it still lands on Deals.
*/
...
export { genericCsvAdapter, metatraderAdapter, mt5DealsAdapter, thinkorswimAdapter, tradingviewAdapter };
export type {
  AdapterBuildResult,
  AdapterContext,
  AdapterMatch,
